...
import { login, selectError, selectStatus } from '../../redux/auth/authSlice';
...
const LoginForm = ({ handleLogin }) => {
  ...
  const handleSubmit = async (evt) => {
    ...
    dispatch(login({ username, password })).then((result) => {
      if (result.payload) {
        Cookies.set('user', JSON.stringify(result.payload.user), { expires: 7 } // Set the token cookie with a 7-day expiration
        );
